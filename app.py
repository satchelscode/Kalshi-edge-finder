    ...
    try:
        ...
        for mkt in markets:
            ...
            for home_abbr, away_abbr, tie_score, guaranteed_total, game_date_str in tied_games:
                ...
                # IMPORTANT: Only Over 2.5 is guaranteed when tied 1-1, NOT Over 3.5+
                print(f"      Checking {ticker}: line={line}, guaranteed_total={guaranteed_total}, tied={tie_score}-{tie_score}")
...
def find_all_resolved_markets(kalshi_api) -> List[Dict]:
    ...
    # 5. Stock index markets (S&P 500, Nasdaq-100) — also scanned by sniper thread near close
    now_et = _get_eastern_now()
